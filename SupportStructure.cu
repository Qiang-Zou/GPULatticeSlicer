...
__global__ void krSLAContouring_CalConnectionMap2(short2 *linkID, int i, int j, bool *bflag, int* linkMapidx,int *cnt, int *pixelNum, int nodeNum, int2 imgRes,
	double nSampleWidth)
{
	int index = threadIdx.x + blockIdx.x*blockDim.x;
	short2 st, ed;
...
__global__ void krSLAContouring_CheckConnectionMap2(short2 *linkID, int i, int j, bool *bflag, int *linkMapidx,int *cnt,int *cntnow, int *pixelNum, int nodeNum, int2 imgRes)
{
	int index = threadIdx.x + blockIdx.x*blockDim.x;
	short2 st, ed;
	int ix, iy;
...
__global__ void krSLAContouring_ConnectionMapOnLayers2(bool* upperLayer,bool* lowerLayer,int layerNum1, bool* tempImg, int* linkMapidx, int cnt, bool* bflagDelete, short2* linkID, int i, int j, int pixelNum, int lengthofLayer, int furtherStepLength, int layerNumforOneCircle, int endlayer, int startlayer, int3 imgRes, int nodeNum,int numofLayer)
{
	int index = threadIdx.x + blockIdx.x*blockDim.x;
	int ix, iy, iz, m;
	short2 st, ed;
	...
	while (index<cnt) {
		int id = linkMapidx[index];
		ix = id % imgRes.x;	iz = id / imgRes.x;
...
//__global__ void Dynamic_Parallelism(int linkNum, bool *linkfilter, short2 *linkID,
//	int resX,int resY,int resZ, double nSampleWidth, int lengthofLayer, int furtherStepLength, 
//	unsigned int *linkLayerC, short *linkLayerD, bool *upperLayer, bool *lowerLayer, bool *lowerSupt, 
//	int layerNum1,int *vLinkcu,int vLinkNum)
//{
//	unsigned int index = threadIdx.x + blockIdx.x*blockDim.x;
//	//int totalNum = linkNum * linkNum;
//	//printf("totalNum: %d",totalNum);
//	int nodeNum = resX * resY;
...
//
//	int *numofPixelcu;
//	cudaStatus=cudaMalloc((void**)&(numofPixelcu), sizeof(int));
//	numofPixelcu[0] = 0;
//	if (cudaStatus != cudaSuccess) {
//		printf("cudaMalloc failed for numofPixelcu!\n");
//	}
//	
//
//	int *linkCount;
//	cudaMalloc((void**)&(linkCount), sizeof(int));
//	linkCount[0] = 0;
//	
//	while (index < vLinkNum) {
//		//if (linkfilter[index]) 
//		{
//			m = vLinkcu[index] % linkNum;	n = vLinkcu[index] / linkNum;
//			numofPixelcu[0] = 0;
//			//printf("index: %d\n", index);
//			
//			bool *linkMap = (bool*)malloc(nodeNum * sizeof(bool));
//			if (linkMap == NULL) {
//				printf("cudaMalloc failed for linkMap!\n");
//				//printf(cudaGetErrorString(cudaGetLastError()));
//			}
//			//for (int i = 0; i < nodeNum; i++)linkMap[i] = false;
//			memset(linkMap, false, nodeNum * sizeof(bool));
//			linkCount[0] = 0;
//
//			krSLAContouring_CalConnectionMap << <BLOCKS_PER_GRID, THREADS_PER_BLOCK >> > (linkID, m, n, linkMap, linkCount, resX * resY, make_int2(resX, resY), nSampleWidth);
//
//			krSLAContouring_CheckConnectionMap << <BLOCKS_PER_GRID, THREADS_PER_BLOCK >> > (linkID, m, n, linkMap, linkCount, resX * resY, make_int2(resX, resY));
//
//			/*thrust::device_ptr<bool> target_ptr(linkMap);
//			numofPixelcu[0] = thrust::count(target_ptr, target_ptr + (resX * resY), 1);*/
//			int sum = 0;
...
//			numofPixelcu[0] = sum;
//			//printf("sum: %d\n",sum);
//			numofPixelcu[0] = numofPixelcu[0] - 2 - 1;
//
//			if (numofPixelcu[0] > lengthofLayer)
//			{
//				st_layer = linkLayerC[m] >= linkLayerC[n] ? linkLayerC[m] : linkLayerC[n];
//				ed_layer = linkLayerD[m] <= linkLayerD[n] ? linkLayerD[m] : linkLayerD[n];
//				numofLayer = abs(st_layer - ed_layer) + 1 - 2 - 1;
//
//				int IY = layerNum1 - st_layer - 2;
//				
//				if (IY < 0 || IY >= numofLayer) {
//					free(linkMap);
//					index += blockDim.x * gridDim.x;
//					continue;
//				}
//				else {
//					if (numofLayer > 0)
//						cudaStatus = cudaMalloc((void**)&(bflagLayer), numofLayer * sizeof(bool));
//					else {
//						free(linkMap);
//						index += blockDim.x * gridDim.x;
//						continue;
...
//							
//				if (cudaStatus != cudaSuccess) {
//					printf("cudaMalloc failed for bflagLayer!\n");
//					printf(cudaGetErrorString(cudaGetLastError()));
//				}
//				//cudaMemset((void*)bflagLayer, false, numofLayer * sizeof(bool));
//				tempLayer = (bool*)malloc(nodeNum * sizeof(bool));
//				if (tempLayer == NULL) {
//					printf("cudaMalloc failed for tempLayer!\n");
//					//printf(cudaGetErrorString(cudaGetLastError()));
//				}
//				memset(tempLayer,false, nodeNum * sizeof(bool));
//				memset(bflagLayer,false, numofLayer * sizeof(bool));
//				/*for (int i = 0; i < nodeNum; i++)
//				{
//					tempLayer[i] = false;
//				}
...
//				}*/
//
//				layerNumforOneCircle = (numofPixelcu[0] - (lengthofLayer - 1) + 1) / furtherStepLength + 1;
//
//				//int ccc = thrust::count(target_ptr, target_ptr + (resX * resY), 1);
//				//printf("num of pixel %d %d %d %d \n", numofPixel[0],numofLayer, layerNumforOneCircle, ccc);
//
//
//				krSLAContouring_ConnectionMapOnLayers2 << <BLOCKS_PER_GRID, THREADS_PER_BLOCK >> > (upperLayer, lowerLayer, layerNum1, tempLayer, linkMap, bflagLayer, linkID, m, n, numofPixelcu[0], lengthofLayer, furtherStepLength, layerNumforOneCircle, ed_layer, st_layer, make_int3(resX, resY, resZ), resX * resY, numofLayer);
//				krSLAContouring_DeleteMapOnLayers2 << <BLOCKS_PER_GRID, THREADS_PER_BLOCK >> > (tempLayer, bflagLayer, layerNumforOneCircle, make_int2(resX, resY), resX * resY, layerNum1, st_layer, numofLayer);
//				krSLAContouring_FilterLink22 << <BLOCKS_PER_GRID, THREADS_PER_BLOCK >> > (lowerSupt, layerNum1, tempLayer, st_layer, ed_layer, make_int3(resX, resY, resZ - 1), resX * resY, numofLayer);
//				free(linkMap);
//				free(tempLayer);
//				cudaFree(bflagLayer);
//
...
//				//cudaFree(tempLayer);
//			}
//			//cudaMemset((void*)linkMap, false, resX * resY * sizeof(bool));
//			//cudaMemset((void*)linkCount, 0, sizeof(int));
//			//cudaFree(linkMap);
//			//linkCount[0] = 0;
//		}
//
	...
	int lengthofLayer, int furtherStepLength, int linkNum, double nSampleWidth,int layerNum1)
{
	...
	CUDA_SAFE_CALL(cudaMemset((void*)linkCount, 0, sizeof(int)));
	int *numofPixelcu;
	CUDA_SAFE_CALL(cudaMalloc((void**)&(numofPixelcu), sizeof(int)));
	CUDA_SAFE_CALL(cudaMemset((void*)numofPixelcu, 0, sizeof(int)));

	int *mLock;
	cudaMalloc((void**)&(mLock), sizeof(int));
	cudaMemset((void*)mLock, 0, sizeof(int));

	/*cudaMalloc((void**)&(tempLayer), imageSize[0] * imageSize[2] * sizeof(bool));
	cudaMemset((void*)tempLayer, false, imageSize[0] * imageSize[2] * sizeof(bool));*/
	

	int time = clock();
	//cudaDeviceSetLimit(cudaLimitMallocHeapSize, 128 * 1024 * 1024);
	printf("totalNum: %d\n",linkNum*linkNum);
	if (layerNum1 == imageSize[1] - 2)
	{
		for (int i = 0; i < linkNum*linkNum; i++)
		{
			if (blink[i])
		...
		vLink = new int[vlink.size()];
		memcpy(vLink, &vlink[0], vlink.size() * sizeof(vlink[0]));

		CUDA_SAFE_CALL(cudaMalloc((void**)&(vLinkcu), vlink.size() * sizeof(vlink[0])));
		CUDA_SAFE_CALL(cudaMemcpy(vLinkcu, vLink, vlink.size() * sizeof(vlink[0]), cudaMemcpyHostToDevice));
	}
	/*Dynamic_Parallelism << <2, 224 >> > (linkNum,linkfilter,linkID,
		imageSize[0], imageSize[2], imageSize[1],nSampleWidth,lengthofLayer,furtherStepLength,
		linkLayerC, linkLayerD, upperLayer, lowerLayer,lowerSupt,layerNum1,vLinkcu, vlink.size());*/
	...
	int layerNumforOneCircle;
	//int time = clock();
	int vlinkSize = vlink.size(); printf("vlinkSize: %d\n",vlinkSize);

	if (layerNum1 == imageSize[1] - 2)
	{
		for (int i = 0; i < vlinkSize; i++)
		{
			m = vlink[i] % linkNum;	n = vlink[i] / linkNum;

			int* cnt;
			CUDA_SAFE_CALL(cudaMalloc((void**)&(cnt), sizeof(int)));
			CUDA_SAFE_CALL(cudaMemset((void*)cnt, 0, sizeof(int)));
			int* linkMapidx;
			CUDA_SAFE_CALL(cudaMalloc((void**)&(linkMapidx), 200*sizeof(int)));
			CUDA_SAFE_CALL(cudaMemset((void*)linkMapidx, 0, 200*sizeof(int)));
			bool *linkMap;
			CUDA_SAFE_CALL(cudaMalloc((void**)&(linkMap), imageSize[0] * imageSize[2] * sizeof(bool)));
			CUDA_SAFE_CALL(cudaMemset((void*)linkMap, false, imageSize[0] * imageSize[2] * sizeof(bool)));

			krSLAContouring_CalConnectionMap2 << <BLOCKS_PER_GRID, THREADS_PER_BLOCK >> > (linkID, m, n, linkMap, linkMapidx, cnt, linkCount, imageSize[0] * imageSize[2], make_int2(imageSize[0], imageSize[2]), nSampleWidth);
			int* cntnow;
			CUDA_SAFE_CALL(cudaMalloc((void**)&(cntnow), sizeof(int)));
			CUDA_SAFE_CALL(cudaMemcpy(cntnow, cnt, sizeof(int), cudaMemcpyDeviceToDevice));
			krSLAContouring_CheckConnectionMap2 << <BLOCKS_PER_GRID, THREADS_PER_BLOCK >> > (linkID, m, n, linkMap, linkMapidx, cnt, cntnow, linkCount, imageSize[0] * imageSize[2], make_int2(imageSize[0], imageSize[2]));
			
			//int *linkMapidx_host = new int[200];
			//CUDA_SAFE_CALL(cudaMemcpy(linkMapidx_host, linkMapidx, 200 * sizeof(int), cudaMemcpyDeviceToHost));
			//std::vector<int> linkMapidx_hostV(linkMapidx_host, linkMapidx_host+200);
			int *cnthost = new int[1];
			CUDA_SAFE_CALL(cudaMemcpy(cnthost, cnt, sizeof(int), cudaMemcpyDeviceToHost));

			linkMapVector.push_back(linkMapidx);
			cntVector.push_back(cnthost[0]);
			cudaFree(linkMap);
			//cudaFree(linkMapidx);
			cudaFree(cnt);
			cudaFree(cntnow);
	...
	for (int i = 0; i < vlinkSize; i++)
	{
		//if (blink[i])
		{
			...
			st_layer = cpu_layerC[m] >= cpu_layerC[n] ? cpu_layerC[m] : cpu_layerC[n];//较大值
			ed_layer = cpu_layerD[m] <= cpu_layerD[n] ? cpu_layerD[m] : cpu_layerD[n];//较小值
			numofLayer = abs(st_layer - ed_layer) + 1 - 2 - 1; // if(this->layerind<startinglayer+2 || this->layerind>= endinglayer)
			...
			//thrust::device_ptr<bool> target_ptr(linkMap);
			//numofPixel[0] = thrust::count(target_ptr, target_ptr + (imageSize[0] * imageSize[2]), 1);
			numofPixel[0] = cntVector[cnt_linkMap];
			...
			if (numofPixel[0] > lengthofLayer)
			{
				...
				int layerID = st_layer + 2 + IY;
				int localID = IY % layerNumforOneCircle;
				if (layerNumforOneCircle - localID > ed_layer - layerID)
				{
					//CUDA_SAFE_CALL(cudaMemset((void*)linkMap, false, imageSize[0] * imageSize[2] * sizeof(bool)));
					//CUDA_SAFE_CALL(cudaMemset((void*)linkCount, 0, sizeof(int)));
					cnt_linkMap++;
				...
				int* linkMapidx = linkMapVector[cnt_linkMap];
				//for (int k = 0; k < linkMapidx_hostV.size(); k++)printf("%d\n", linkMapidx_hostV[k]);
				//int* linkMapidx_host = new int[200];
				//memcpy(linkMapidx_host, &linkMapidx_hostV[0], linkMapidx_hostV.size() * sizeof(linkMapidx_hostV[0]));
				//int* linkMapidx;
				//CUDA_SAFE_CALL(cudaMalloc((void**)&(linkMapidx), 200 * sizeof(int)));
				//CUDA_SAFE_CALL(cudaMemcpy(linkMapidx, linkMapidx_host,  200 * sizeof(int), cudaMemcpyHostToDevice));
				int cnt = cntVector[cnt_linkMap];

				krSLAContouring_ConnectionMapOnLayers2 << <BLOCKS_PER_GRID, THREADS_PER_BLOCK >> > (upperLayer, lowerLayer, layerNum1, tempLayer, linkMapidx, cnt, bflagLayer, linkID, m, n, numofPixel[0], lengthofLayer, furtherStepLength, layerNumforOneCircle, ed_layer, st_layer, make_int3(imageSize[0], imageSize[2], imageSize[1]), imageSize[0] * imageSize[2], numofLayer);
