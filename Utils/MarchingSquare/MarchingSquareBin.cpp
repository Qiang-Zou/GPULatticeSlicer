...
//////////////////////////////////////////////////////////////////////
MarchingSquareBin::MarchingSquareBin(int** &m_image_, size_t xres_, size_t zres_,
...
void MarchingSquareBin::FindAllSticks()
{
    ...
    for(int i=0; i<xres; i++) {
        for(int j=0; j<zres-1; j++)  {
            bool v1(m_image[i][j] == 0);
            bool v2(m_image[i][j+1] == 0);
            if(v1 == !v2) {
                stickindexx->Add(i);
                stickindexx->Add(i);
...
void MarchingSquareBin::basicContouring()
{
    ...
    while(dealtsticknum!=currsticknum) {
        ...
        while(true) {
            ...
            if(startx==endx) {
                if(startz<endz) {
                    bool v1(m_image[endx + 1][endz] == 0);
                    bool v2(m_image[startx + 1][startz] == 0);
                    if(v1) {
                        nextstartx = endx+1;
                        nextstartz = endz;
                        nextendx = endx;
                ...
                if(startx<endx)  {
                    bool v1(m_image[endx][endz - 1] == 0);
                    bool v2(m_image[startx][startz - 1] == 0);
                    if(v1)  {
                        nextstartx = endx;
                        nextstartz = endz-1;
                        nextendx = endx;
...
void MarchingSquareBin::BiLaplacianSmoothOp(double smootherrorcoeff, double movementcoeffict)
{
    ...
    double **temnodescoord = new double*[nodenum];
